...
def test_dummyexporter_with_projects_v2_event(client: TestClient, mocker: MockerFixture) -> None:
    # given
    body = {"sender": {"login": "asdfgsdf"}, "action": "delete", "projects_v2": {"title": "test"}}
    headers = {"x-github-event": "projects_v2"}

    # when
    spy = mocker.spy(DummyExporter, "handle_event")
...
def test_dummyexporter_post_with_delete_event_filterred(client: TestClient, mocker: MockerFixture) -> None:
...
def test_dummyexporter_with_unknown_default_event(client: TestClient, mocker: MockerFixture) -> None:
    # given
    body = {"hello": {"iam": "john"}}
    ...
    assert response.status_code == 204
    spy.assert_called_once()
    spy.assert_called_with(client.app.state.exporters[0], "hello", "{'hello': {'iam': 'john'}}")  # type: ignore
